...
Для подсчета колчиства строк в файле содержащих исколмую строку правильно в данном случае будет использовать grep с параметром -c
...
При выводе списка процессов командой ps -aux можно видеть, что родителем явлется /sbin/init, но если выполним команду
...
Мы можем перенаправить вывод stderr используя следующий синтексис: 2>/dev/pts/x, где x - номер эмулятора терминала.
...
Можно. Используя операторы перенаправления ввода ">","<".
