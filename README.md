...
Предположим, команда разработчиков создала программный продукт, который умеет выполнять операцию  2 + 2. 
Команда тестировщиков выяснила, что в некоторых случаях результат работы программного продукта отличается от предполагаемого. 
Но deadline уже прошел, и клиент требует результат.
Команда маркетологов смогла исправить ситуацию и преподнесла данный баг как фичу, а именно: 

	"Молодая, но очень перспективная команда разработчиков смогла избавиться от такого атавизма как идемпотентность,
	и результат стал удивительным и не предсказуемым". 

Применение IaaC в данной команде позволило в установленные сроки получить среду разработки, тестирования и передать продукт в продакшн и каждый раз результат данных действий был предсказуем.
...
На мой взгляд метод push более предпочтителен, так как принятие решения о запуске сценариев происходит с одного места, сервера управления. Но с другой стороны все зависит от конкретной случая и политики безопасности. Возможна ситуация, когда подключение по ssh к серверам запрещено, но разрешено подключения с серверов к серверу управления, тогда остается метод pull.

## Задача 3

#### Решение:

VirtualBox и Vagrant были установлены раннее в рамках выполнения предыдущих заданий:
...
Ошибка при копировании ключа связана с отсутствие ключа.
